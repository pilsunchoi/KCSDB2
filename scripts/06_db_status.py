...
INVENTORY = {
    ...
    "fact_ip": ("산업별 광공업생산지수(2020=100) 월 계열. 생산·출하·재고의 원지수와 "
                "계절조정, 내수·수출 출하지수, 명절·조업일수 조정 총지수. 산업은 "
                "한국표준산업분류 소분류(3자리)까지이고 1995.01부터 전부 있다",
                "산업 × 지표 × 월",
                "국가데이터처 광업제조업동향조사 · KOSIS DT_1F02001·DT_1F02016·DT_1F02061"),
    "fact_cli": ("경기종합지수(선행·동행·후행, 순환변동치)와 구성지표. 지수는 1970년, "
                 "구성지표 원계열은 2003년부터",
                 "표 × 지표 × 월", "국가데이터처 경기종합지수(10차) · KOSIS DT_1C8015·DT_1C8016"),
    "fact_xmpi": ("수출·수입물가지수(2020=100) 기본분류 월 계열. 달러·원화·계약통화 세 기준. "
                  "교역조건지수(순상품·소득)도 여기 있다. 1971년부터",
                  "방향 × 기준 × 품목 × 월",
                  "한국은행 ECOS 402Y014·401Y015·403Y005"),
    ...
    "dim_hsk_revision": ("개정별 신설·폐지 코드와 사유. 개정 시점이 문서로 확정된다",
                         "(개정, 변경, HS10)",
                         "연도별 별표 차분 + 신구대조표 · 국가법령정보센터·재정경제부"),
    ...
    "dim_ksic": ("생산지수의 산업 코드 사전. 총지수·대분류·중분류·소분류 4층과 상위 코드",
                 "산업", "국가데이터처 KOSIS DT_1F02001 항목 메타"),
    "dim_cli": ("경기종합지수 지표 코드의 이름", "(표, 지표)", "국가데이터처 KOSIS 항목 메타"),
    "dim_xmpi": ("수출입물가지수 품목 코드의 이름·상위 코드·층·2020년 가중치·시작 월",
                 "(방향, 품목)", "한국은행 ECOS 항목 목록 + 응답의 가중치"),
...
PERIOD_COL = {"fact_trade": "yyyymm", "fact_total": "yyyymm", "fact_nqi": "yyyymm",
              "fact_temper": "yyyymm", "fact_ip": "yyyymm", "fact_cli": "yyyymm",
...
def release_body(s: dict, inv: list[dict]) -> list[str]:
    ...
    body = [
        ...
        *group("mart", "mart — 이 저장소가 계산한 지표 (받은 자료가 아니다)"), "",
        "## 알아 둘 것",
        "",
        "- 표마다 기간이 다르다. 가장 긴 것은 경기종합지수(`fact_cli`) 1970년이고, "
        "성질별 공식 실적(`fact_temper`)과 광공업생산지수(`fact_ip`)는 1995년, "
        "신성질별(`fact_nqi`)은 2005년부터다. 조업일수 달력(`dim_workday10d`)은 "
        ...
        "근거를 확인할 것.",
        "- 무역 통계가 아닌 표도 있다 — `fact_ip`(광공업생산지수)와 "
        "`fact_cli`(경기종합지수)는 국가데이터처 KOSIS 자료로, 수출입을 국내 "
        "생산·경기와 맞대 보라고 함께 담았다.",
