...
큰 데이터를 다룰 때는 몇 가지 함정을 미리 규칙으로 고정해야 한다. 상세는 대시보드의 "데이터 함정" 탭과 `docs/session-notes.md`에 있다.
...
두 해(2007·2025) 중 **한 번이라도 수출 상위 10위에 든 나라**를 나란히 놓았다. 각국의 그 해 실제 순위를 양쪽 모두 표기했으므로(10위 밖이면 그 순위 그대로), 어떤 나라가 올라오고 어떤 나라가 밀려났는지가 다 보인다.
...
- **크게 올라온 나라**: 베트남(13위 → 3위, 58억 → 628억으로 10배 이상), 인도(11 → 8), 호주(15 → 9), 말레이시아(14 → 10).
- **상위 10위에서 밀려난 나라**: 멕시코(9 → 11), 독일(7 → 15), 영국(10 → 20), 러시아(8 → 28). 특히 러시아·영국은 크게 내려갔다.
